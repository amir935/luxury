...
const commission_1 = require("../config/commission");
...
const quoteSchema = zod_1.z.object({
    ...
    dropoff: zod_1.z
        ...
        .optional(),
    vehicleClass: zod_1.z.enum(["SILVER", "GOLD", "PLATINUM"]),
...
function trafficMultiplierFromETA(etaTrafficMin, etaFreeflowMin) {
    if (!etaFreeflowMin)
        return 1.0;
...
async function getRouteWithTraffic(pickup, dropoff) {
...
const requestSchema = zod_1.z.object({
    pickup: zod_1.z.object({ address: zod_1.z.string(), lat: zod_1.z.number(), lng: zod_1.z.number() }),
    dropoff: zod_1.z.object({ address: zod_1.z.string(), lat: zod_1.z.number(), lng: zod_1.z.number() }),
    distanceKm: zod_1.z.number().positive(),
    vehicleClass: zod_1.z.enum(["SILVER",
        "GOLD",
        "PLATINUM",]),
});
router.post("/", auth_1.requireAuth, (0, rbac_1.allowRoles)("RIDER", "ADMIN", "SUPER_ADMIN"), async (req, res) => {
    const riderId = req.userId;
    const payload = requestSchema.parse(req.body);
    const ride = await (0, rides_service_1.createRide)({ riderId, ...payload });
    if (process.env.AUTO_ASSIGN === "true") {
        try {
            await (0, dispatch_service_1.dispatchRide)(ride.id, 8);
        }
        catch (e) {
...
/* --------------------------- driver availability ----------------------- */
router.post("/driver/availability", auth_1.requireAuth, (0, rbac_1.allowRoles)("DRIVER"), async (req, res) => {
    ...
    const profile = await prisma_1.prisma.driverProfile.upsert({
        where: { userId },
        update: {},
        create: { userId, licenseNumber: `DRV-${userId}`, isVerified: false, isActive: false },
    });
    if (isActive) {
        if (!profile.isVerified) {
            return res.status(409).json({ error: "Driver must be verified before activation" });
        }
        const approvedVehicle = await prisma_1.prisma.vehicle.findFirst({
            where: { driverId: profile.id, isApproved: true },
        });
        if (!approvedVehicle) {
            return res.status(409).json({ error: "No approved vehicle found" });
        }
    }
...
/* ------------------------------ driver accept -------------------------- */
router.post("/:rideId/accept", auth_1.requireAuth, (0, rbac_1.allowRoles)("DRIVER"), async (req, res) => {
    const userId = req.userId;
    const { rideId } = zod_1.z.object({ rideId: zod_1.z.string() }).parse(req.params);
    const profile = await prisma_1.prisma.driverProfile.findUnique({ where: { userId } });
    if (!profile)
        return res.status(400).json({ error: "No driver profile" });
    ...
    // find an approved vehicle **matching class**
    const vehicle = await prisma_1.prisma.vehicle.findFirst({
        where: { driverId: profile.id, isApproved: true, class: ride.requestedClass }, // ensure ride has requestedClass
    });
    if (!vehicle)
        return res.status(400).json({ error: "No approved vehicle matching requested class" });
    const updated = await prisma_1.prisma.ride.update({
        where: { id: ride.id },
...
/* ----------------------------- update status --------------------------- */
router.post("/:rideId/status", auth_1.requireAuth, (0, rbac_1.allowRoles)("DRIVER"), async (req, res) => {
    const driverUserId = req.userId;
    const { rideId } = zod_1.z.object({ rideId: zod_1.z.string() }).parse(req.params);
    const { status } = zod_1.z.object({ status: zod_1.z.enum(["DRIVER_ARRIVING", "IN_PROGRESS", "COMPLETED", "CANCELED"]) }).parse(req.body);
    const ride = await prisma_1.prisma.ride.findUnique({
        where: { id: Number(rideId) },
        include: { vehicle: true }
    });
    ...
    if (status === "COMPLETED") {
        ...
        const fare = new client_1.Prisma.Decimal((ride.finalPrice ?? ride.estimatedPrice).toString());
        const rate = new client_1.Prisma.Decimal((0, commission_1.getCommissionRate)(ride.vehicle?.class).toFixed(4));
        const fee = fare.mul(rate);
        const driverEarnings = fare.sub(fee);
        data.finalPrice = fare;
        data.commissionRate = rate;
        data.platformFee = fee;
        data.driverEarnings = driverEarnings;
        // CASH-only: rider settles with driver; mark PENDING until reconciled
        data.riderPaymentStatus = "PENDING";
        data.paidAt = null;
        // Wallet effects for CASH:
        // driver owes platform the commission (DEBIT)
        const wallet = await prisma_1.prisma.driverWallet.upsert({
            where: { userId: driverUserId },
            update: {},
            create: { userId: driverUserId, balance: new client_1.Prisma.Decimal(0) }
        });
        await prisma_1.prisma.$transaction([
            prisma_1.prisma.ride.update({ where: { id: ride.id }, data }),
            prisma_1.prisma.walletLedger.create({
                data: {
                    userId: driverUserId,
                    rideId: ride.id,
                    type: "DEBIT",
            ...
            prisma_1.prisma.driverWallet.update({
                where: { id: wallet.id },
                data: { balance: new client_1.Prisma.Decimal(wallet.balance.toString()).sub(fee) }
            })
        ]);
        const updated = await prisma_1.prisma.ride.findUnique({ where: { id: ride.id } });
        return res.json({ ride: updated });
    }
    const updated = await prisma_1.prisma.ride.update({ where: { id: ride.id }, data });
    res.json({ ride: updated });
});
/* --------------------------- ADMIN assign driver ----------------------- */
/** NOTE: path fixed to avoid /rides/rides/... when mounted at /rides */
router.post("/:rideId/assign", auth_1.requireAuth, (0, rbac_1.allowRoles)("ADMIN", "SUPER_ADMIN"), async (req, res) => {
    const { rideId } = zod_1.z.object({ rideId: zod_1.z.string() }).parse(req.params);
    const { driverId, radiusKm = 8 } = zod_1.z
        .object({ driverId: zod_1.z.number().optional(), radiusKm: zod_1.z.number().optional() })
        .parse(req.body);
    const ride = await prisma_1.prisma.ride.findUnique({ where: { id: Number(rideId) } });
    ...
    if (!targetDriverUserId) {
        ...
        const list = candidates
            ...
            .sort((a, b) => a.km - b.km);
        if (!list.length)
            return res.status(404).json({ error: "No nearby available drivers" });
    ...
    if (!vehicle)
        return res.status(400).json({ error: "Driver has no approved vehicle" });
    const updated = await prisma_1.prisma.ride.update({
        where: { id: ride.id },
        data: {
...
/* -------------------------------- cancel ------------------------------- */
router.post("/:rideId/cancel", auth_1.requireAuth, (0, rbac_1.allowRoles)("RIDER"), async (req, res) => {
    const { rideId } = zod_1.z.object({ rideId: zod_1.z.string() }).parse(req.params);
...
router.get("/history", auth_1.requireAuth, (0, rbac_1.allowRoles)("RIDER"), async (req, res) => {
    const riderId = req.userId;
    const { limit = "20", offset = "0", q, sort = "created_at", order = "desc" } = req.query;
    ...
    const orderBy = sort === "fare"
        ? [{ finalPrice: order }, { estimatedPrice: order }]
        : sort === "time"
            ? [{ startedAt: order }]
            : [{ requestedAt: order }];
    const [total, rides] = await Promise.all([
        prisma_1.prisma.ride.count({ where }),
    ...
    const items = rides.map((r) => {
        const ride_time = r.startedAt && r.completedAt
...
/* ----------------------------- driver history -------------------------- */
router.get("/driver/history", auth_1.requireAuth, (0, rbac_1.allowRoles)("DRIVER"), async (req, res) => {
    const driverId = req.userId;
    const { limit = "20", offset = "0", q, sort = "created_at", // "created_at" | "fare" | "time"
    order = "desc", // "asc" | "desc"
        ...
        return {
            ...
            rider: r.rider
                ? {
                    ...
                    name: r.rider.name,
                    profile_image_url: r.rider?.avatarUrl ?? null, // safe until types are fresh
                }
                : null,
...
/* -------------------------- basic lists (old) -------------------------- */
router.get("/me", auth_1.requireAuth, (0, rbac_1.allowRoles)("RIDER"), async (req, res) => {
    const riderId = req.userId;
...
router.get("/driver/me", auth_1.requireAuth, (0, rbac_1.allowRoles)("DRIVER"), async (req, res) => {
    const driverId = req.userId;
    const rides = await prisma_1.prisma.ride.findMany({
        where: { driverId },
        orderBy: { id: "desc" },
...
router.get("/seed/history", auth_1.requireAuth, (0, rbac_1.allowRoles)("RIDER", "ADMIN", "SUPER_ADMIN"), async (req, res) => {
    ...
    const skip = Math.max(0, Number(offset) || 0);
    const [total, rides] = await Promise.all([
        prisma_1.prisma.ride.count({ where: { riderId } }),
        prisma_1.prisma.ride.findMany({
            where: { riderId },
    ...
    const driverIds = Array.from(new Set(rides.map(r => r.driverId).filter(Boolean)));
