...
// Use Prisma runtime Decimal consistently
...
router.post(
  ...
  async (req, res) => {
    ...
    // NOTE: ensure createRide persists requestedClass on Ride
    const ride = await createRide({ riderId, ...payload });

    if (process.env.AUTO_ASSIGN === "true") {
      try {
        await dispatchRide(ride.id, 8);
      } catch (e) {
  ...
  async (req, res) => {
    ...
    if (isActive) {
      if (!profile.isVerified) {
        return res
          .status(409)
          .json({ error: "Driver must be verified before activation" });
      }
      const approvedVehicle = await prisma.vehicle.findFirst({
...
router.post(
  ...
  async (req, res) => {
    const userId = (req as any).userId as number;
    const { rideId } = z.object({ rideId: z.coerce.number().int().positive() }).parse(req.params);
    ...
    if (ride.status !== "REQUESTED")
      return res
        .status(409)
        .json({ error: `Ride is ${ride.status}, cannot accept` });

    // Must have an approved vehicle matching the ride's requested class
    const vehicle = await prisma.vehicle.findFirst({
      where: {
        driverId: profile.id,
        isApproved: true,
        class: ride.requestedClass,
      },
    });
    if (!vehicle)
      return res
        .status(400)
        .json({ error: "No approved vehicle matching requested class" });
...
router.post(
  ...
  async (req, res) => {
    ...
    const { rideId } = z
      .object({ rideId: z.coerce.number().int().positive() })
      .parse(req.params);
    const { status } = z
      .object({
        status: z.enum([
    ...
    if (status === "IN_PROGRESS" && !ride.startedAt) data.startedAt = now;
    if (status === "CANCELED" && !ride.canceledAt) data.canceledAt = now;

    if (status === "COMPLETED") {
      data.completedAt = now;

      const fare = new Decimal(
        (ride.finalPrice ?? ride.estimatedPrice).toString()
      );
      const rate = new Decimal(
        getCommissionRate(ride.vehicle?.class as any).toFixed(4)
      );
      ...
      // CASH-only: the driver collected from rider → platform awaits remit
      ...
      const wallet = await prisma.driverWallet.upsert({
        ...
        update: {},
        create: { userId: driverUserId, balance: new Decimal(0) },
      });
      ...
      await prisma.$transaction([
        ...
        prisma.driverWallet.update({
          where: { id: wallet.id },
          data: {
            balance: new Decimal(wallet.balance.toString()).sub(fee),
          },
        }),
      ]);
...
router.post(
  ...
  async (req, res) => {
    ...
    if (!targetDriverUserId) {
      const candidates = await prisma.driverProfile.findMany({
        where: {
          ...
          currentLat: { not: null },
          currentLng: { not: null },
          // Must have at least one approved vehicle of the requested class
          vehicles: {
            some: { isApproved: true, class: ride.requestedClass },
          },
        },
        include: {
          vehicles: {
            where: { isApproved: true, class: ride.requestedClass },
            take: 1,
          },
        },
      });
    ...
    const vehicle = await prisma.vehicle.findFirst({
      where: {
        driver: { userId: targetDriverUserId! },
        isApproved: true,
        class: ride.requestedClass, // ensure class match
      },
    });
    if (!vehicle)
      return res
        .status(400)
        .json({ error: "Driver has no approved vehicle of requested class" });
    ...
    const updated = await prisma.ride.update({
      ...
      data: {
        ...
        vehicleId: vehicle.id,
        status: "ASSIGNED",
        acceptedAt: new Date(), // (optional) if you prefer, add separate assignedAt
      },
      ...
      return {
        ...
        rider: r.rider
          ? {
              ...
              name: r.rider.name,
              profile_image_url: (r.rider as any)?.avatarUrl ?? null,
            }
          : null,
...
router.get(
  ...
  async (req: any, res) => {
    ...
    const fmtTime = (dt: Date) => {
      ...
      const ms = String(dt.getMilliseconds()).padStart(3, "0") + "000";
    ...
    const items = rides.map((r) => {
      const ride_time =
        r.startedAt && r.completedAt
          ? Math.round((r.completedAt.getTime() - r.startedAt.getTime()) / 60000)
          : 0;
      const [first_name = "Driver", ...rest] = (r.driver?.name ?? "").split(" ");
      const last_name = rest.join(" ");
      return {
        ride_id: String(r.id),
        origin_address: r.pickupAddress,
